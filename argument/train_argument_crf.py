...
dataset = TargetNERDataset("ner_data/argument_ner_train.jsonl", label2id, tokenizer)
dataloader = DataLoader(dataset, batch_size=16, shuffle=True)

model = BERTCRFForNER(model_name, num_labels=len(label2id)).cuda()
optimizer = torch.optim.AdamW(model.parameters(), lr=2e-5)

val_dataset = TargetNERDataset("ner_data/argument_dev.jsonl", label2id, tokenizer)
