...
def extract_target_spans(tokens, pred_labels):
    spans = []
    curr = []
...
def predict(contents):
    ...
    for content in tqdm(contents):
        ...
        targets = extract_target_spans(tokens_list, pred_labels)
        results.append({"content": content, "pred_targets": targets})

    return results

if __name__ == "__main__":
    with open("ner_data/target_dev.jsonl", encoding="utf-8") as f:
        test_inputs = ["".join(json.loads(line)["tokens"]) for line in open("ner_data/target_dev.jsonl", encoding='utf-8')]

    results = predict(test_inputs)
    with open("predict_target_results.json", "w", encoding="utf-8") as f_out:
